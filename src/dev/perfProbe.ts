...
function scrollBurstTargets(doc: Document): ScrollBurstTarget[] {
	...
	return [
		target(
			'tree',
			'.vaultman-tree-virtual-viewport, .vm-tree-virtual-outer',
			'.vaultman-tree-row, .vm-tree-virtual-row:not([data-sticky="true"])',
			...
			{
				icon: '.vaultman-tree-icon, .vm-tree-icon',
				label: '.vaultman-tree-label, .vm-tree-label',
				field: '.vaultman-tree-type, .vm-tree-field',
				count: '.vaultman-tree-count, .vm-tree-count',
				badge:
					'.vaultman-tree-badge-zone .vaultman-badge, .vm-tree-badge-zone .vm-badge, .vm-tree-overlay-badge-zone .vm-badge',
			},
		),
		target(
			'list',
		...
		target(
			'table',
			'.vaultman-node-table-scroll, .vaultman-files-table, .vm-node-table',
			...
			{
				icon: '.vaultman-node-table-icon, .vaultman-file-icon, .vm-node-table-icon',
				label: '.vaultman-node-table-label, .vaultman-file-name, .vm-node-table-primary',
				cell: '.vaultman-node-table-type, .vaultman-file-ext, .vaultman-file-date, .vaultman-file-path, .vm-node-table-cell',
				badge:
					'.vaultman-node-table-badge-zone .vaultman-badge, .vaultman-file-table-badge-zone .vaultman-badge, .vm-node-table-badge-zone .vm-badge',
			},
		),
		target(
			'grid',
			'.vaultman-files-grid-scroll, .vm-node-grid',
			'.vaultman-files-grid-card, .vm-node-grid-tile[data-id], .vm-node-grid-row',
			...
			{
				icon: '.vaultman-files-grid-card-icon, .vm-node-grid-icon',
				label: '.vaultman-files-grid-card-name, .vm-node-grid-label',
				field: '.vaultman-files-grid-card-ext, .vm-node-grid-field',
				badge:
					'.vaultman-card-badge-zone .vaultman-badge, .vm-node-grid-badge-zone .vm-badge, .vm-node-grid-hover-badge-zone .vm-badge',
...
function isInsideInactiveTabContent(element: HTMLElement): boolean {
	const tabContent = element.closest<HTMLElement>(
		'.vaultman-filters-tab-pane, .vm-tab-content',
	);
		...
		await measureAsync(`scenario.${name}.duration`, scenarioInput, async () => {
			if (name === 'filters-search') {
				const input = doc?.querySelector<HTMLInputElement>(
					'.vaultman-filters-search-input, .vm-filters-search-input',
				);
				...
				const row = doc?.querySelector<HTMLElement>(
					'.vaultman-tree-row, .vm-tree-virtual-row',
				);
