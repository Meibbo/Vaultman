...
export const shortcutsNavigation: UserShortcuts = [
	...
	['vm-nav-pill-active', 'bg-[var(--interactive-accent)] text-[var(--text-on-accent)] font-semibold shadow-xs'],
	...
	['vm-nav-icon', 'flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-normal)] transition-colors duration-150'],
	['vm-toolbar', 'flex items-center gap-1.5 p-1 border-b border-[var(--background-modifier-border)] bg-[var(--background-primary)]'],
	['vm-dock', 'fixed bottom-0 left-0 right-0 z-30 flex items-center justify-center gap-2 p-2 bg-[var(--background-primary)]/80 backdrop-blur-md border-t border-[var(--background-modifier-border)]'],
];
