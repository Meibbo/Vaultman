...
/**
 * AC#1 import-boundary guard for the Q4 logic-extraction (slice 2).
 *
 ...
 * `src/logic/logicProps.ts` and assert no module in that graph imports `obsidian`
 * (or anything ending in `.svelte`). Cheap, deterministic, runs in `node`.
 * Mirrors `logicFilesBoundary.test.ts`.
 */
...
describe('logicProps import boundary (AC#1)', () => {
	const entry = resolve(srcRoot, 'logic/logicProps.ts');

	it('logicProps.ts exists', () => {
