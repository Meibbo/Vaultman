...
/**
 * AC#1 import-boundary guard for the Q4 logic-extraction (slice 4 — logicBadge).
 *
 ...
 * `src/logic/logicBadge.ts` and assert no module in that graph imports `obsidian`
 * (or anything ending in `.svelte`). Cheap, deterministic, runs in `node`.
 * Mirrors `logicTagsBoundary.test.ts` / `logicPropsBoundary.test.ts`.
 */
...
describe('logicBadge import boundary (AC#1)', () => {
	const entry = resolve(srcRoot, 'logic/logicBadge.ts');

	it('logicBadge.ts exists', () => {
