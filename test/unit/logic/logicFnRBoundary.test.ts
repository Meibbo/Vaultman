...
/**
 * AC#1 import-boundary guard for the Q4 logic-extraction (slice 5 — logicFnR).
 *
 ...
 * `src/logic/logicFnR.ts` and assert no module in that graph imports `obsidian`
 * (or anything ending in `.svelte`). Cheap, deterministic, runs in `node`.
 * Mirrors `logicBadgeBoundary.test.ts` / `logicTagsBoundary.test.ts`.
 */
...
describe('logicFnR import boundary (AC#1)', () => {
	const entry = resolve(srcRoot, 'logic/logicFnR.ts');

	it('logicFnR.ts exists', () => {
