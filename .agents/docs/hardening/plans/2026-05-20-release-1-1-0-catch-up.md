...
Release catch-up completed on 2026-05-20 to unblock Obsidian submission by replacing old `main`
with a sanitized `sandbox` tree and bootstrapping release automation.

## Outcome

- `main` reconciled to `sandbox - AI files`; `main` contains zero `AGENTS.md`, `CLAUDE.md`,
  `.agents/`, `.claude/`, or `.codex/` files.
- `main-clean-1.1.0` candidate was verified green after gating the optional Notebook Navigator
  comparison fixture.
- Release infrastructure landed through PR #20:
  ...
  - `release-please` runs from `main` with bare tags and `manifest.json` / `versions.json`
    extra-files.
...
- Release assets published: `main.js`, `manifest.json`, `styles.css`, `SHA256SUMS`,
  `sbom.cdx.json`.
- Scorecard publish failure was fixed through PR #22 by disabling OpenSSF webapp publishing while
  keeping SARIF upload.
...
- Dependabot security alerts on default branch were resolved or auto-dismissed after the sanitized
  main replacement; remaining Dependabot PRs are routine updates unless reopened as vulnerabilities.
...
- Next implementation entrypoint: A.R Action Routing detail spec, then `writing-plans`, then
  `v1.2.0` execution.
- Remote cleanup is optional: `main-clean-1.1.0` and `release/1.1.0-infra` are merged/stale
  release branches created during this work.
