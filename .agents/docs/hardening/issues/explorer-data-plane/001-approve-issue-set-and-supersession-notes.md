...
- [x] The old `serviceViews` implementation plan is marked historical,
      partially completed, or otherwise not executable as-is.

## Resolution

- User approved using the local Markdown issue set as the operational tracker
  for the Explorer data-plane transition.
- Tracker target remains
  [[docs/work/hardening/issues/explorer-data-plane/index|Explorer data plane local issues]].
- Label vocabulary is recorded in the tracker index and now includes
  `completed` for resolved local issues.
- Supersession source is
  [[docs/work/hardening/specs/2026-05-11-explorer-data-plane-structural-taxonomy/18-wave-5-plan-comparison-reconciliation|Wave 5 plan comparison and reconciliation]]:
  `NodeSelectionService` owns selection/focus/hover/active state.
- The old
  [[docs/work/hardening/plans/2026-05-04-serviceviews-implementation/index|serviceViews implementation plan]]
  is historical and partially completed. It is not executable as the current
  Explorer data-plane plan.
...
- [x] Re-read linked source specs and confirm no stale implementation plan is
