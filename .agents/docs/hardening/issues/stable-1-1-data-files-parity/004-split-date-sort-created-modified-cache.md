...
- Props runtime smoke switched to Props and clicked both date-derived sorts:
  `Modified time` found and rendered in about `909 ms`; `Created time` found and rendered in about
  `241 ms`; `dev:errors` stayed clean.

## Completion Notes

- `normalizeExplorerSortBy('date')` migrates legacy state to `mtime`.
- `StatisticsCacheService` now persists `ctime` with `mtime`, `size`, word count, links, props,
  values, and tags in the per-file cache record.
- Props and Tags no longer compute date sort with nested `nodes x markdownFiles` loops. They build a
  per-render timestamp index using `statisticsCache.getFileTimes(file)` when available and `file.stat`
  as fallback.
- The `Modified time` smoke is not a freeze, but the measured first render is still a performance
  watch item if users report it as jank.
