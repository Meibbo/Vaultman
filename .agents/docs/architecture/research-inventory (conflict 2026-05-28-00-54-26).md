...
| Dynamic Views recon + Bases-view ecosystem + Path A/B | bases-interop-findings (ecosystem + strategy section) | ~6–10 plugins on the API; recommend **HYBRID** (PROPOSED) |
| serviceMark persistence | this doc + chat 2026-05-27 | **serviceMark NOT built yet** (0 refs). Durable pattern = `saveData()`→`data.json` (pageOrder, leaf-detach), synced by default. No RAM-only marks exist. |
