...
Document which PKM-AI tools are authoritative, which are heuristic, and which need deeper Modules before agents rely on them for architecture claims.
...
Create the file with the same control-plane shard frontmatter and title `Agent control plane - tool contracts`.
...
- `archive-active-doc.mjs`: authoritative for archive creation when command succeeds and the archive link is present;
...
Completed 2026-05-10T05:26:33. Created [[docs/work/pkm-ai/specs/2026-05-10-agent-control-plane/06-tool-contracts|Tool contracts]] with confidence levels, current PKM-AI tool classifications, and use rules.
Linked the shard from the Agent Control Plane spec index and added the compact `Tool Confidence` addendum to [[docs/architecture/policies/tools|Tools policy]].

Verification:

- `node .agents\tools\pkm-ai\check-doc-health.mjs | Select-String -Pattern 'tool-contracts|tools.md|doc health'` still reports global `doc health: FAIL (47)`, with no `tool-contracts` or `tools.md` path-specific failure.
- `Select-String -Path '.agents/docs/architecture/policies/tools.md', '.agents/docs/work/pkm-ai/specs/2026-05-10-agent-control-plane/06-tool-contracts.md' -Pattern '[ \t]+$'` returned no matches.
- Scoped `git diff --check` returned no path errors; Git emitted only CRLF conversion warnings.
