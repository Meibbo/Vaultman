...
Validate Pi-crew in an isolated temporary project before writing the PKM-AI `agent-room` spec.

The spike tested installation and durable state primitives only. It did not run LLM worker sessions, did not execute a real Pi team run, and did not modify Vaultman product code.
...
Then a Node smoke imported Pi-crew's installed TypeScript modules through `jiti` from the temporary local Pi package directory and exercised state APIs:
...
`pi install npm:pi-crew -l` succeeded in the temporary project and installed Pi-crew under:
...
- When a project already has `.pi/settings.json`, Pi-crew stores team state in `.pi/teams`, matching the legacy layout documented by the package.
...
- Worker heartbeat can be represented as JSON and mirrored into per-agent status.

## Limits

- The spike did not run `/team-run`, `/team-status`, `/team-dashboard`, or a real child Pi worker.
- No provider API call was made.
- Worktree mode was not tested.
- The smoke imported package internals through `jiti`; that validates the contract shape but is not an endorsed public integration boundary.
...
- avoid depending on Pi-crew as the runtime for Codex, Claude, Gemini, or Antigravity sessions.

The next document should be a PKM-AI `agent-room` spec, not a Pi-crew adoption plan.
