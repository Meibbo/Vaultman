...
Post-catch-up: cómo arrancar v1.2.0 + qué docs actualizar + handoff a writing-plans.
...
- No NodeRow primitive build (eso es N.R en v1.3.0; A.R define contract, N.R consumes)
- No View Decomposition (eso es V.D en v1.3.0)
- No viewTree sticky-parents fix (eso es sibling spec en v1.2.0 — `viewTree-sticky-fix-spec`)
...
- Update "Next Action" para reflejar: A.R first detail spec, then plan, then v1.2.0 execution
...
## Parallel agent dispatch para v1.2.0
...
11. Approved pipeline v1.2.0 → v2.0.0 final ordering after `1.1.0` catch-up
