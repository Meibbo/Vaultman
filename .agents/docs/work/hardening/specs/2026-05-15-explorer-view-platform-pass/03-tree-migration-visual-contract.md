...
The `viewTree` work is a real migration to the platform architecture, but it is not a visual remake.

The correct baseline is the expected tree contract, not the currently broken tree state. Known regressions must be fixed as part of the migration because they are user-visible and currently make the tree feel broken.
...
Filtered nodes should use accent as a semantic signal without becoming eye-candy or reducing label readability.
...
- implementation must verify and reuse the current accent transparency source before changing CSS.
...
- any accent tint must use the existing translucent accent treatment and must not overpower label readability;
- behavior is documented and covered by visual/component tests.

### File Extension Placement

File extensions should be aligned at the far right of the row, not at an intermediate point.
...
- `.md` is hidden because it is implied and the props counter already carries the relevant markdown/file signal.
...
If implementation discovers a tempting cleanup outside this list, it should be recorded as follow-up polish rather than bundled into the migration.

## Required Baselines

Before product migration begins, capture the expected tree states as tests or snapshots:
...
The migration is done only when these states preserve or restore the expected contract while moving the data/scroll/geometry responsibilities out of the current god object.
