...
7. [[08-value-operations|Value operations: labels, `Add to files`, property type]]
   + [[08-value-operations-part-2|part 2: the `Move to prop...` mode]]
8. [[09-reveal-this-file-properties|`reveal this file` properties mode]]
9. [[next-agent-prompt|Self-contained next-agent prompt]]
...
Shards 08 and 09 were written on 2026-08-02 against `claude/u121-029-panel-widget`
at `26499dde`, after plan shards 01–05 had landed, so they plan against the
contracts that now exist rather than the state at amendment time:

- 08 — value operations, from
  [[../spec-2026-08-02-corrective-primitives/07-value-operations|spec shard 07]],
  split into [[08-value-operations|labels, `Add to files`, property type]] and
  [[08-value-operations-part-2|the `Move to prop...` mode]]. Depends on the
  OperationTargetSet of plan shard 03 and the capability resolver of plan shard
  04, so it runs after both.
- 09 — [[09-reveal-this-file-properties|`reveal this file` properties mode]],
  from
...
Spec shard 07 §B says the move mode replaces the `reveal this file` ActionNode,
which shard 09 introduces. The circularity is resolved by having shard 08 build
the **slot** and its precedence rule, and shard 09 put reveal into that slot and
re-point the mutual-exclusion test. Neither blocks the other.
