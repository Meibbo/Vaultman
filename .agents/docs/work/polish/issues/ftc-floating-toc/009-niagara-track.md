...
Restore a coherent bell-shaped slide by letting action and group nodes share one
geometry track when requested, while reducing beta.1 to supported appearance options.
...
- [x] Component source contracts + pure target/suppression tests prove action scrub
      cannot invoke callbacks while a quick action tap remains available.
...
- `pnpm run check`: 0 errors / 0 warnings; production bundle, ESLint, Stylelint,
  targeted Svelte formatting, and `git diff --check` passed.
- Global `format:check` remains red only for 18 pre-existing Svelte files outside this
  diff; both modified Svelte files pass targeted Prettier.

## Non-goals

Deleting deferred implementation code · device feel/performance verdict · new 2.0
ActionNode machinery.
