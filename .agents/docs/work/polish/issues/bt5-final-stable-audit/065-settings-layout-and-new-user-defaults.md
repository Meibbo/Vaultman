...
Move the Layout Configuration section ahead of the templates / operation-sets /
saved-compositions sections, and soften two defaults for new vaults only: the
compact preset without tab labels, and one-click cancel for operation badges.

## Acceptance criteria

- [x] Layout Configuration is the first heading on the settings root page.
- [x] `filtersShowTabLabels` defaults off; the one-time label migration still
      keys off a saved value, so existing vaults are untouched.
...
Resolved by `cf9ba359`. Defaults apply to fresh vaults only; `plugin-dev`
keeps its saved `data.json` values.
